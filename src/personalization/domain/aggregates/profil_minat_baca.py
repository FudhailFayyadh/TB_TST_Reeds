...
class ProfilMinatBaca:
    ...
    user_id: UserId
    preferensi_eksplisit: PreferensiEksplisit = field(
        default_factory=PreferensiEksplisit.empty
    )
    ...
    def block_item(self, book_id: str) -> None:
        ...
        if (
            book_id in self.riwayat_baca
            and self.riwayat_baca[book_id].rating is not None
        ):
            raise ValueError(
                f"Cannot block active book: {book_id}. Remove rating first."
            )
        ...
        self.domain_events.append(
            ItemDiblokir(
                user_id=str(self.user_id), book_id=book_id, timestamp=datetime.utcnow()
            )
        )
