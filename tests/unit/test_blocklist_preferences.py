...
from src.personalization.domain.value_objects.preferensi_eksplisit import (
    PreferensiEksplisit,
)
...
class TestDaftarBlokir:
    ...
    def test_contains_blocked_book(self):
        ...
        assert blokir.contains("book-bad")
        assert not blokir.contains("book-good")
...
class TestPreferensiEksplisit:
    ...
    def test_has_genre(self):
        ...
        assert pref.has_genre("Fantasy")
        assert not pref.has_genre("Horror")
    ...
    def test_create_with_non_genre_favorit_fails(self):
        ...
        with pytest.raises(
            ValueError, match="All items must be GenreFavorit instances"
        ):
