...
class TestJWTTokenCreation:
    ...
    def test_custom_expiration(self):
        ...
        token = create_access_token(
            data={"sub": "user-001"}, expires_delta=timedelta(seconds=5)
        )
...
class TestAuthEndpointSecurity:
    ...
    def test_password_not_returned_on_login(self, client):
        ...
        response = client.post(
            "/auth/login", data={"username": "testuser", "password": "secretpassword"}
        )
...
class TestProtectedEndpointSecurity:
    ...
    def test_invalid_auth_scheme_rejected(self, client):
        ...
        response = client.get(
            "/profile/user-001", headers={"Authorization": "Basic dXNlcjpwYXNz"}
        )
        assert response.status_code == 401

    def test_empty_bearer_token_rejected(self, client):
        """Empty bearer token should be rejected"""
        response = client.get("/profile/user-001", headers={"Authorization": "Bearer "})
        assert response.status_code == 401

    def test_malformed_jwt_rejected(self, client):
        """Malformed JWT should be rejected"""
        response = client.get(
            "/profile/user-001", headers={"Authorization": "Bearer not.a.valid.jwt"}
        )
        assert response.status_code == 401
    ...
    def test_forged_jwt_rejected(self, client):
        ...
        response = client.get(
            "/profile/admin", headers={"Authorization": f"Bearer {forged_token}"}
        )
        assert response.status_code == 401
...
class TestAuthorizationSecurity:
    ...
    def attacker_auth(self, client):
        ...
        resp = client.post(
            "/auth/register", json={"username": "attacker", "password": "evil123"}
        )
        user_id = resp.json()["user_id"]
        response = client.post(
            "/auth/login", data={"username": "attacker", "password": "evil123"}
        )
    ...
    def victim_auth(self, client):
        ...
        resp = client.post(
            "/auth/register", json={"username": "victim", "password": "innocent123"}
        )
        user_id = resp.json()["user_id"]
        response = client.post(
            "/auth/login", data={"username": "victim", "password": "innocent123"}
        )
        token = response.json()["access_token"]
        return {"headers": {"Authorization": f"Bearer {token}"}, "user_id": user_id}

    def test_attacker_cannot_create_victim_profile(
        self, client, attacker_auth, victim_auth
    ):
        """Attacker cannot create profile for another user"""
        response = client.post(
            f"/profile/{victim_auth['user_id']}", headers=attacker_auth["headers"]
        )
        assert response.status_code == 403

    def test_attacker_cannot_view_victim_profile(
        self, client, attacker_auth, victim_auth
    ):
        """Attacker cannot view victim's profile"""
        # Victim creates their profile
        client.post(
            f"/profile/{victim_auth['user_id']}", headers=victim_auth["headers"]
        )
    ...
    def test_attacker_cannot_add_genre_to_victim(
        self, client, attacker_auth, victim_auth
    ):
...
class TestTokenManipulationAttacks:
    ...
    def test_none_algorithm_attack(self, client):
        ...
        response = client.get(
            "/profile/admin", headers={"Authorization": f"Bearer {none_token}"}
        )
        assert response.status_code == 401
    ...
    def test_sql_injection_in_user_id(self, client, valid_token):
        ...
        response = client.get("/profile/'; DROP TABLE users;--", headers=headers)
    ...
    def test_path_traversal_in_user_id(self, client, valid_token):
        ...
        response = client.get("/profile/../../../etc/passwd", headers=headers)
